...
class SaleModel extends BaseModel {
    ...
    function register($username, $password, $email, $other = array(), $rand) {
        ...
        if (!ECTouch::user()->add_user($username, $password, $email)) {
            ...
            //定义other合法的变量数组
            $other_key_array = array('msn', 'qq', 'office_phone', 'home_phone', 'mobile_phone', 'parent_id','user_rank');
    ...
    public function get_line_count(){
        $count = M()->table('users')->field('COUNT(*)')->where("parent_id = ".session('user_id'))->getOne();
     ...
     */
    public function get_line_list($num, $start){

        $sql = 'SELECT * FROM ' . $this->pre . "users WHERE parent_id = " . session('user_id') . " ORDER BY user_id DESC limit " . $start . ',' . $num;
        $res = $this->query($sql);
        ...
        foreach ($res as $k => $v) {
            ...
            $res[$k]['reg_time']    =   local_date(C('date_format'), $v['reg_time']);
            $res[$k]['mobile_phone']    =    substr_replace($v['mobile_phone'],'****',3,4);
        }
